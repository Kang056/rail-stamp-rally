-- Migration: Add checkin_logs table for daily check-in tracking
-- Created: 2026-04-21

-- 1. 建立 checkin_logs 表格（每日每站限打卡 1 次）
...
-- 唯一約束：同一使用者同一車站同一天只能打卡一次
CREATE UNIQUE INDEX IF NOT EXISTS uq_checkin_logs_user_station_date
  ON checkin_logs (user_id, station_id, checkin_date);

CREATE INDEX IF NOT EXISTS idx_checkin_logs_user_id
  ON checkin_logs (user_id);

-- 2. 更新 checkin() RPC 加入每日限制 + 寫入 checkin_logs
...
BEGIN
  -- Find nearby station within 100 m
...
-- 3. New RPC: get_user_checkin_count
CREATE OR REPLACE FUNCTION get_user_checkin_count(p_user_id uuid)
RETURNS integer
LANGUAGE sql
STABLE
SECURITY INVOKER
AS $$
  SELECT COUNT(*)::integer FROM checkin_logs WHERE user_id = p_user_id;
$$;

-- 4. RLS
ALTER TABLE checkin_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can read own checkin_logs"
...
-- 5. Grant
GRANT SELECT, INSERT ON checkin_logs TO authenticated;
GRANT EXECUTE ON FUNCTION get_user_checkin_count(uuid) TO authenticated;
